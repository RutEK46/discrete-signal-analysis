...
\begin{equation}
\begin{array}{lll}
upper \: band_{(n, k)}(X) & = & SMA_{n}(X) + k\cdot\sigma^2_{n}(X) \\
middle \: band_{(n, k)}(X) & = & SMA_{n}(X) \\
bottom \: band_{(n, k)}(X) & = & SMA_{n}(X) - k\cdot\sigma^2_{n}(X),
\end{array}
\end{equation}
gdzie:
\begin{equation}
\begin{array}{lll}
SMA_{n}(X) & = & \frac{1}{n}\sum_{i = 0}^n(X_i) \\
\sigma^2_{n}(X) & = & \frac{1}{n}\sum_{i = 0}^n(X_i - SMA_n(X))^2
\end{array}
\end{equation}
...
\end{itemize}
W naszym przypadku wykorzystujemy funkcjê nazywaj¹c¹ siê sigmoid. Gdy zwrócona przez sigmoid wartoœæ bêdzie wiêksza od 0.5, jest to sygna³ do zakupu akcji dla danego wskaŸnika, a gdy wartoœæ jest mniejsza ni¿ 0.5 - sygna³ do sprzedania. Wagi pozwalaj¹ stwierdziæ jak wa¿na ma byæ decyzja danego wskaŸnika i w przypadku tego wskaŸnika, je¿eli wartoœæ naszego zagregowanego wskaŸnika jest wiêksza ni¿ 0, jest to sygna³ do kupna, a w przeciwnym wypadku do sprzeda¿y.
...
Algorytm genetyczny jest algorytmem heurystycznym do znajdowania prawie optymalych rozwi¹zañ w sytuacjach, gdy szukanie rozwi¹zania optymalnego jest trudne i samo rozwi¹zanie prawie optymalne nas satysfakcjonuje. Algorytm jest wzorowany na dzia³aniu ewolucji genetycznej u zwierz¹t w naturze. W algorytmie genetycznym wykorzystuje siê nastêpuj¹ce pojêcia:
\begin{itemize}
...
tutaj zauwa¿ymy czy wyniki s¹ zgodne z naszymi oczekiwaniami, poka¿emy dane - kiedy s¹ zadowalaj¹ce wyniki 
Uzyskalimy ciekawe efekty, gdy¿ algorytm genetyczny da³ ujemne wyniki. Byæ mo¿e jest to spowodowane...
